...
object InvestigatorDetailsContract {
    data class State(
        ...
        val investigator: Cached<Pair<ArkhamHorrorExpansion, ArkhamHorrorExpansion.Investigator>> = Cached.NotLoaded(),
    )

    sealed class Inputs {
        data class Initialize(val investigatorId: String) : Inputs()
        data class InvestigatorUpdated(val investigator: Cached<Pair<ArkhamHorrorExpansion, ArkhamHorrorExpansion.Investigator>>) : Inputs()
    }
