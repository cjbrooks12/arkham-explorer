...
interface InputHttpNode : Node.Input {
    ...
    private fun cachedResponseFile(graph: DependencyGraph): Path {
        return graph.config.httpCacheDir / "${url.replace("\\W+".toRegex(), "_")}/${url.md5}/response.json"
    }

    private fun cachedMetadataFile(graph: DependencyGraph): Path {
        return graph.config.httpCacheDir / "${url.replace("\\W+".toRegex(), "_")}/${url.md5}/metadata.json"
    }
